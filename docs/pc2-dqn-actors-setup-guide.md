...
## Самый простой способ (GUI-лаунчер ПК2)
На ПК2 вместо `.bat` можно запустить окно:
```bat
cd C:\40kAI
.\.venv\Scripts\python tools\pc2_launcher.py
```
В окне: вписать **общую папку ПК1** (`40KAI_SHARE_ROOT`, напр. `\\PC1\40kai_models`),
нажать **«Проверить SMB»**, выбрать роль (**DQN актора** / AZ актора / AZ-IS / GMZ-IS)
и **«Старт»**. Лог — в том же окне. Ниже — ручной путь через `.bat`.
...
- **Ростер/миссию на ПК2 синхронить вручную не нужно:** ПК1 кладёт снимок ростера
  в `dqn_dist_train_context.json` (SMB), ПК2 строит env ровно из него. Локальный
  `runtime/state/data.json` на ПК2 — только фолбэк (если ПК1 старый, без снимка).

## Шаги
1. **ПК1:** в Qt GUI вкладка **Настройка параметров тренировки** → секция гиперпараметров **DQN**
   → панель **DISTRIBUTED ACTORS** сверху → включи **Distributed actors (ПК2)**
   (порт `distributed_rollout_port` оставь 5558, `distributed_auth_token` — опц.).
   Сохрани hyperparams и запусти train. Learner поднимет
...
2. **ПК2:** запусти `tools\pc2_dqn_actors.bat` — при первом запуске он скопирует
   ...
   В нём **обязательно задать только `40KAI_SHARE_ROOT`** — путь к общей SMB-папке ПК1
   (напр. `\\192.168.1.10\40kai_models`; можно указать и прямо на `...\actor_sync`).
   Остальное выводится автоматически:
   - **IP ПК1** ← из UNC-пути шары,
   ...
   > **Совместимость:** старая переменная `MODELS_DIR` тоже понимается (если
   > `40KAI_SHARE_ROOT` пуст). Приоритет: `40KAI_SHARE_ROOT` → `40KAI_MODELS_DIR` →
   > `MODELS_DIR`. Та же переменная унифицирована для AZ distributed actors
   > (`pc2_remote_az_is_config.bat` / `pc2_az_actors`).
...
- Если `drop invalid message` / `env_contract_hash mismatch` — ростер теперь едет
  из контекста ПК1, так что причина обычно в **разной версии кода/ruleset** на ПК1≠ПК2
  (`git pull` на обеих) либо в строке `ростер: локальный …data.json` в логе ПК2
  (значит снимка в контексте нет — ПК1 старой версии).
