...
**На ПК2** (inference server):

1. Проводник → **Этот компьютер** → вкладка **Компьютер** → **Подключить сетевой диск**.
2. Диск: **`Z:`** (любая свободная буква).
3. Папка: `\\DESKTOP-ABC\actor_sync` — подставьте **имя ПК1** и имя share из шага 2.  
   Пример: `\\192.168.1.10\actor_sync` (можно по IP вместо имени).
4. Галочка **Восстанавливать подключение при входе** — по желанию.
5. Если спросит логин/пароль — учётная запись **ПК1** (тот же пользователь, которому дали доступ).

6. Проверка в cmd на ПК2:
   ```bat
   dir Z:\
