...
## P0.2 / P0.3 (статус: выполнено)
- Добавлен `eval_score` на базе window-метрик:
  - `win_rate`, `draw_rate`, `wipeout_enemy_rate`, `wipeout_model_rate`, `vp_diff_norm`.
- При улучшении `eval_score` сохраняется `best_eval_checkpoint.pth`, лог:
  - `[TRAIN][BEST] ... old_score -> new_score ...`.
- Добавлен safety-guard от regressions:
  - при устойчивом `draw_pit` автоусиление `TURN_LIMIT_DRAW_PENALTY`,
  - автоувеличение `pool_old_prob` для более сложных historical opponents,
  - лог: `[TRAIN][GUARD] anti_draw_adjustment ...`.

## P1.1 (статус: выполнено)
- Для pool-score добавлены EMA-компоненты:
  - `ema_win`, `ema_draw`, `ema_vp_diff`.
- Добавлены параметры:
  - `SELF_PLAY_POOL_EMA_ENABLED`
  - `SELF_PLAY_POOL_EMA_ALPHA`
  - `SELF_PLAY_POOL_MIN_GAMES_FOR_SMART`
- Score для smart-sampling теперь:
  - использует EMA (если включено),
  - сглаживается по confidence при малом числе игр (защита от шумовых переоценок новых оппонентов).
