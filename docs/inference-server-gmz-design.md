...
## 15. Remote Inference Server (B-remote)

Реализовано отдельно от B-local (см. `docs/remote-inference-server-gmz.md`):

- **Транспорт:** ZMQ DEALER (env worker) / ROUTER (ПК2), msgpack + numpy bytes.
- **Клиент:** `core/models/gmz_inference_transport.py`, `GMZInferenceClient` с `transport=`.
- **Сервер:** `tools/gmz_remote_inference_server.py` (standalone), reuse `GMZInferenceServer.build_batch_responses`.
- **Веса:** SMB polling `latest_gmz_policy.pth` (тот же путь, что learner на ПК1).
- **GUI:** Настройки → Gumbel MuZero → панель Inference Server; конфиг `runtime/state/remote_is.json`.
- **Маркеры:** `[GMZ][REMOTE_IS]`, `[GMZ][REMOTE_CLIENT]`.
