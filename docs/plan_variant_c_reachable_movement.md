...
## План внедрения (итерация: быстрый win без смены action-space)

Ниже план на ближайшие спринты: сначала усиливаем Variant C в текущем action-space, затем готовим hybrid cell-head, потом rollout и только после этого чистый cell-based режим.

### Этап A — Быстрый win в Variant C (без изменения action-space)

#### A1. Penalty за `stay/no-move` при отсутствии валидной стрельбы
**Что делаем**
- В movement-фазе для MODEL считаем флаг `no_shoot_after_move`:
  - `1`, если юнит выбрал `stay/no-move` и после movement в shooting-фазе нет валидных целей в effective range.
- Добавляем отдельный shaping-компонент `stay_no_shoot_penalty` (мягкий, с cap).

**Зачем**
- Сейчас в логах есть `hold_penalty` + повторяющиеся случаи `цель вне дальности` после стагнации.

**Логи/метрики**
- Новый лог: `[MOVE][QUALITY] unit=... stay=1 no_shoot_after_move=1 penalty=-... reason=...`
- Счётчики в конце эпизода:
  - `stay_count`,
  - `stay_without_shoot_count`,
  - `stay_without_shoot_rate`.

**Критерий готовности**
- `stay_without_shoot_rate` падает на сидовом батче без ухудшения VP/winrate.

#### A2. Бонус за вход в effective range
**Что делаем**
- Добавляем reward-компонент `enter_effective_range_bonus`:
  - до movement: цели вне effective range,
  - после movement: хотя бы одна цель в effective range,
  - бонус ограничен (cap), чтобы не перебить objective-логику.

**Зачем**
- Нужен прямой стимул ходить не только к objective, но и в боевую дистанцию.

**Логи/метрики**
- `[MOVE][QUALITY] unit=... entered_effective_range=1 bonus=+... range_before=... range_after=...`
- Агрегаты:
  - `entered_effective_range_count`,
  - `out_of_range_shoot_events` (до/после).

**Критерий готовности**
- `out_of_range_shoot_events` снижается, средний урон/полезные шоты не падают.

#### A3. Разделить intent-цели: `to_objective` vs `to_shoot_range`
**Что делаем**
- В scorer Variant C добавить отдельную компоненту для intent `to_shoot_range`.
- Intent policy обучать с mix-целью:
  - objective progress,
  - range entry,
  - threat/cover баланс.

**Зачем**
- Убрать конфликт, когда юнит “правильно” идёт к objective, но системно теряет огневой темп.

**Логи/метрики**
- `[MOVE][INTENT] unit=... intent=to_objective|to_shoot_range score_obj=... score_range=... chosen=...`

**Критерий готовности**
- Доля намерений `to_shoot_range` появляется там, где цель близка к порогу дальности.

**Статус этапа A (выполнено)**
- ✅ A1 реализован: добавлен `stay/no-move` penalty при `no_shoot_after_move` + метрики/логи `[MOVE][QUALITY]`.
- ✅ A2 реализован: добавлен бонус за вход в effective range + cap на суммарный range bonus.
- ✅ A3 реализован: в Variant C scorer добавлен intent `to_shoot_range`, а `engage_target` маппится в `to_shoot_range` для плавного перехода без смены action-space.

---

### Этап B — Hybrid cell-head + top-K candidates (плавный переход)

#### B1. Добавить hybrid голову выбора клетки
**Что делаем**
- Не удаляем legacy/Variant C selector.
- Добавляем опциональный `move_cell_selector_i` (или shared head с index per unit).
- Policy предсказывает ranking/score по candidate set.

#### B2. Top-K compression
**Что делаем**
- Из полного `C = move_cells ∪ advance_cells ∪ stay` берём top-K кандидатов по быстрому heuristic pre-score.
- RL head выбирает только среди K (например K=8/12/16).

**Зачем**
- Снижает размер action-space и шум градиентов, ускоряет обучение.

#### B3. Fallback-first стратегия
**Что делаем**
- Если confidence cell-head низкий или mask пустой:
  - fallback на текущий Variant C scorer.
- Флаги:
  - `MOVEMENT_CELL_HEAD_MODE=off|shadow|live`
  - `MOVEMENT_CELL_HEAD_TOPK=...`

**Критерий готовности этапа B**
- В `shadow` расхождение с baseline объяснимо, в `live` нет деградации по ключевым метрикам.

**Статус этапа B (базово выполнено)**
- ✅ B1 реализован: добавлен optional head `move_cell_selector_i` (включается при `MOVEMENT_CELL_HEAD_MODE=shadow|live`).
- ✅ B2 реализован: selection идёт через top-K (`MOVEMENT_CELL_HEAD_TOPK`), затем выбор внутри top-K.
- ✅ B3 реализован: fallback-first — при `off`/отсутствии selector используется scorer Variant C; в `shadow` cell-head только логируется; в `live` применяется.
- ✅ Добавлены логи `[MOVE][CELL_HEAD]` с mode/selector/topk/reason.

---

### Этап C — Rollout: shadow -> live -> parity

#### C1. Shadow-прогон
- Гоняем минимум N матчей на фиксированном seed-наборе.
- Логируем:
  - legacy choice,
  - variant choice,
  - hybrid choice,
  - expected delta (obj/range/threat/cover).

#### C2. Частичный live rollout
- Включение по доле эпизодов: 10% -> 25% -> 50% -> 100%.
- Авто-откат при деградации выше порога.
...
- KPI:
  ...
  - VP delta,
  - objective progress,
  - out-of-range shoot rate,
  - stay_without_shoot_rate,
  - exposure/received damage.

**Критерий готовности этапа C**
- `hybrid_live` не хуже `variant_c_live` по winrate/VP и лучше по movement quality.

---

### Этап D — Полный cell-based режим (без fallback)

Переходим только если этапы A/B/C закрыты.

**Что делаем**
- Делаем cell-based выбор основным контрактом.
- Fallback оставляем только как аварийный kill-switch на rollout.
- Перезаписываем train/eval дефолты под cell-head.
...
## Технический backlog (конкретные задачи)

1. Reward-конфиг:
- `MOVE_STAY_NO_SHOOT_PENALTY`
- `MOVE_ENTER_EFFECTIVE_RANGE_BONUS`
- `MOVE_RANGE_BONUS_CAP`

2. Movement meta поля:
- `stay_without_shoot`
- `entered_effective_range`
- `range_to_best_target_before/after`

3. Train summary / results:
- добавить агрегаты quality-метрик movement.

4. Лог-маркеры (единый формат):
- `[MOVE][DEBUG]` (уже есть),
- `[MOVE][QUALITY]`,
- `[MOVE][INTENT]`,
- `[MOVE][CELL_HEAD]`.
...
5. Rollout safety:
- пороги деградации и авто-rollback флагов.

---

## Рекомендуемый порядок внедрения (коротко)

1) A1 -> A2 -> A3 (быстрые win'ы в текущем Variant C)
2) B1 + B2 + B3 (hybrid и top-K)
3) C1 -> C2 -> C3 (shadow/live/parity)
4) D (чистый cell-based без fallback)

Так мы получаем заметное улучшение поведения уже на раннем этапе и минимизируем риск регрессий перед полным переходом.
