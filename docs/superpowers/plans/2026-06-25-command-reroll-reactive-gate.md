...
**Уточнение архитектора 2026-06-25 (обязательно для реализации Task 4):**
- `_command_reroll_wasted_penalty()` → всегда 0: CP структурно не теряется после реактивного гейта, премиса штрафа исчезла. Функцию/поле reward оставить (back-compat), значение всегда 0 — это НЕ reward-редизайн, а нейтрализация ставшего невалидным штрафа.
- НЕ штрафовать armed-not-fired: arm теперь бесплатный, штраф учил бы НЕ армить → терялись бы полезные рероллы провалов.
- Метрику `cmd_reroll_wasted` ОСТАВИТЬ по имени (не ломать парсеры), но в логе/комментарии уточнить смысл: «armed-not-fired, CP не потрачен» — это диагностика, не штрафуемый waste.
- `fired` = реальные рероллы провалов. Тест: после фазы `applied`(arm) может быть > `fired`, но `penalty=0`.
- Структурный реактивный гейт ЗАМЕНЯЕТ прежний shaped `command_reroll_wasted_penalty` (он покрывал только DQN/PPO; теперь не нужен).
