...
| **B3-full** reaction value-policy | ✅ | Спека `specs/2026-06-19-b3-reaction-value-policy-design.md`, план `plans/2026-06-19-b3-reaction-value-policy.md`. Все 8 задач: флаг `reaction_value_policy` (дефолт 0); `reaction_value_policy.py` (net-value apply/pass, тай→PASS); harness `_simulate_reaction_branch` + recursion guard + seam `resolve_trigger`; decision-пламбинг 4 реакций; **Task 8** — install в AZ-актора (az_net) + env-worker (evaluator-адаптер) + eval (model→net); все call-site triggers через `_set_shot_reaction_trigger` (defensive cover / offensive overwatch) + heroic counter-charge (фикс stale-ссылки на *InAttack). **Гейты:** 170 тестов зелёные; smoke flag=1 `[AZ][ACTOR] reaction_value_policy=ON` без краша (обе стороны); **50ep parity flag=0 Δ=0pp** (legacy .16 / windowed .16). Дефолт 0 → продакшн/parity не затронут. |
