...
| B3 reactions | 🟡 | Слой reaction-окон готов (Stage 8.4g): `reaction_windows.build_reaction_windows` (opt-in `WINDOWED_REACTION_WINDOWS=0`, PASS+legal reaction-стратагемы), тест `test_reaction_windows.py` (4). Reaction-стратагемы (overwatch/smokescreen/heroic_intervention/go_to_ground) исполняются auto-логикой движка. Добавлены research-комменты Wahapedia (core stratagems) для всех 4 + помечены sandbox-упрощения (overwatch=hits_on_6, smoke/go_to_ground=только Benefit of Cover, heroic=counter_charge). Полная интеграция reaction-окон в self-play/MCTS — Stage 8.5+ (требует brainstorming, отдельно). |
