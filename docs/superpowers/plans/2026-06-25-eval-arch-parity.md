...
Агенты, обученные ДО этого фикса, не имеют `arch` в meta → строятся на env-дефолтной арке (симметрично для обеих сторон). 1:1 для недефолтной арки гарантируется только для агентов, обученных ПОСЛЕ Task 6. Если у такого legacy-агента реальная арка несовместима по shape с env-дефолтом — `_safe_load` (Task 2) отдаст **фатальную понятную RU-ошибку** (не тихий мусор): «арка не совпадает, переобучите/перерегистрируйте с arch». Это осознанный выбор честного eval (fail-fast > деградация). Альтернатива (инференс арки из формы state_dict, как `infer_dqn_arch_from_state_dict`) сняла бы и это ограничение — отдельный тикет, в план не входит.

---

## File Structure

- `core/models/eval_agent.py` — **новый** `resolve_arch_for_algo()`, **новый** `_safe_load()` (lenient + size-mismatch→понятная ошибка); правка `build_eval_agent` (param `log_fn`, GMZ-ветка на arch+lenient).
...
- [ ] **Step 1: Написать падающий тест (GMZ: 1:1 чисто; missing/unexpected → WARN; shape mismatch → понятная ошибка)**

> **Важно (ревью дирижёра, 2026-06-25):** `torch.nn.Module.load_state_dict(strict=False)` пропускает только **missing/unexpected** ключи; на **совпадающих ключах с разным shape** PyTorch всё равно бросает `RuntimeError: size mismatch`. Поэтому «рассинхрон арки той же семьи (latent 8 vs 256)» — это НЕ WARN, а краш. Архитектурное решение: shape mismatch → **фатальная понятная RU-ошибка** (не маскируем мусорной сетью — это противоречит честному eval); WARN остаётся для missing/unexpected (переименованные/legacy-слои), которые грузятся lenient без краша.
...
def test_build_eval_agent_gmz_missing_unexpected_warns():
    ...
    # WARN про missing/unexpected, БЕЗ краша. Это та «тихая деградация», которую делаем видимой (Q3).
    env = build_env()
    env.reset(options={"m": env.model, "e": env.enemy, "trunc": True})
    n_obs = len(env.get_observation_for_side("model"))
    n_actions = action_sizes_from_env(env, len(env.model))
...
> `import torch` нужен в шапке тест-файла (для `torch.zeros`); добавить вместе с использованием (ruff-хук).

- [ ] **Step 2: Запустить — убедиться, что падает**

Run: `python -m pytest tests/models/test_eval_arch_parity.py -q -k gmz`
Expected: FAIL — `build_eval_agent() got an unexpected keyword argument 'log_fn'`; а тест `..._shape_mismatch_raises` падает с сырым `RuntimeError: Error(s) in loading state_dict ... size mismatch` (без RU-обёртки) и не матчится по `не совпадает`.
...
(3a-bis) Добавить модуль-функцию `_safe_load` (рядом с `build_eval_agent`, до неё). Все 4 lenient-загрузчика имеют единую сигнатуру `load_*(net, state, *, log_fn=None) -> (missing, unexpected)`; PyTorch на разном shape совпадающих ключей бросает `RuntimeError` с подстрокой `size mismatch` даже при `strict=False`. Обёртка превращает её в понятную RU-ошибку:
...
- WARN-видимость (Q3) → Task 2: log_fn в загрузчиках + `_safe_load` (missing/unexpected → WARN; **size mismatch → фатальная RU-ошибка**, не маскируем) + Task 4 (log_fn=log обеим сторонам). ✓

**Ревью дирижёра (2026-06-25) — учтено:** `load_state_dict(strict=False)` падает на size-mismatch совпадающих ключей. Принято решение (вариант 2): shape mismatch → фатальная понятная ошибка через `_safe_load`; WARN — только для missing/unexpected. Task 2 тесты переписаны под это (`_missing_unexpected_warns`, `_shape_mismatch_raises`).
