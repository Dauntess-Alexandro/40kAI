...
### Статус AZ/GAZ (поправка)

AZ (tree+proxy) и GAZ **уже включены по умолчанию** — это проверено по коду, не требует правок:
`reaction_value_policy: 1` в `hyperparams.json` для tree (`:133`) и proxy (`:204`); резолв
`train.py:2398-2402` имеет код-дефолт `1` и пишет результат в `os.environ`. Инлайновые
`os.getenv("AZ_REACTION_VALUE_POLICY", "0")` (`train.py:6049`, `:6322`) — фолбэк, перетираемый
строкой 2402 (видимый «0» вводит в заблуждение, реально OFF не наступает). GAZ берёт код-дефолт `1`
(в секции `gumbel_az` ключа нет).

Опциональная косметика (defense-in-depth, не меняет поведение): инлайновые `getenv(...,"0")` →
`"1"`, чтобы видимый дефолт совпадал с реальным. Включается в план как отдельный мелкий шаг.
