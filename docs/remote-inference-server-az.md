...
> MCTS-дерево и env-rollout'ы остаются на ПК1. Порт по умолчанию **5555** (тот же, что у GMZ).
> Не запускайте AZ и GMZ remote-сервер на одном ПК2 одновременно (конфликт порта);
> по очереди — ок. Нужны оба разом — задайте разные порты в конфиге/GUI.
...
│ Env workers (CPU)   │─── infer req (ZMQ :5555) ───────────►│ net.infer (priors,  │
...
python -c "from core.models.az_inference_transport import az_remote_health_check; print(az_remote_health_check(host='192.168.1.100', port=5555))"
```
...
| health_check failed с ПК1 | сервер запущен? firewall TCP 5555? верный IP? |
