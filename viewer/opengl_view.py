...
class OpenGLBoardWidget(QOpenGLWidget):
    ...
    def _draw_units_layer(self, painter: QtGui.QPainter) -> None:
        for render in self._units:
            ...
            icon = render.icon
            if model_centers:
                # Мульти-аура: мягкие точки под каждой моделью, чтобы строй читался лучше.
                aura_color = QtGui.QColor(render.color)
                aura_color.setAlpha(55)
                painter.setBrush(QtGui.QBrush(aura_color))
                painter.setPen(QtCore.Qt.NoPen)
                aura_radius = max(3.0, self.cell_size * self._model_icon_scale * 0.42)
                for model_center in model_centers:
                    painter.drawEllipse(model_center, aura_radius, aura_radius)

                # Лёгкий общий эффект (без «портала» по центру) вокруг формации.
                xs = [c.x() for c in model_centers]
                ys = [c.y() for c in model_centers]
                if xs and ys:
                    group_rect = QtCore.QRectF(
                        min(xs) - aura_radius,
                        min(ys) - aura_radius,
                        (max(xs) - min(xs)) + aura_radius * 2,
                        (max(ys) - min(ys)) + aura_radius * 2,
                    )
                    group_color = QtGui.QColor(render.color)
                    group_color.setAlpha(28)
                    painter.setBrush(QtGui.QBrush(group_color))
                    painter.setPen(QtCore.Qt.NoPen)
                    painter.drawRoundedRect(group_rect, aura_radius, aura_radius)
    ...
    def _draw_selection_layer(self, painter: QtGui.QPainter) -> None:
        if self._selected_unit_key in self._unit_by_key:
            ...
            model_centers = render.model_centers or []
            if model_centers:
                # Внешний контур формации вместо центрального круга.
                xs = [c.x() for c in model_centers]
                ys = [c.y() for c in model_centers]
                pad = max(4.0, self.cell_size * 0.35)
                contour = QtCore.QRectF(
                    min(xs) - pad,
                    min(ys) - pad,
                    (max(xs) - min(xs)) + pad * 2,
                    (max(ys) - min(ys)) + pad * 2,
                )
                painter.drawRoundedRect(contour, pad * 0.7, pad * 0.7)
            else:
    ...
    def _draw_fx_layer(self, painter: QtGui.QPainter) -> None:
        ...
        if active_render:
            ...
            model_centers = active_render.model_centers or []
            if model_centers:
                # Внутренний пульс по нескольким ключевым моделям для active-юнита.
                pulse_indices = sorted({0, len(model_centers) // 2, len(model_centers) - 1})
                local_glow_size = max(self.cell_size * 1.1, glow_size * 0.42)
                local_ring_size = max(self.cell_size * 0.95, ring_size * 0.34)
                local_glow_alpha = glow_alpha * 0.75
                local_ring_alpha = ring_alpha * 0.8
                for idx in pulse_indices:
                    center = model_centers[idx]
                    painter.save()
                    painter.setCompositionMode(QtGui.QPainter.CompositionMode_SourceOver)
                    self._draw_fx_sprite(painter, glow_pixmap, center, local_glow_size, local_glow_alpha)
                    painter.restore()
                    painter.save()
                    painter.setCompositionMode(QtGui.QPainter.CompositionMode_Plus)
                    self._draw_fx_sprite(painter, ring_pixmap, center, local_ring_size, local_ring_alpha)
                    painter.restore()
            else:
                painter.save()
                painter.setCompositionMode(QtGui.QPainter.CompositionMode_SourceOver)
