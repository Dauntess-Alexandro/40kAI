...
class ViewerWindow(QtWidgets.QMainWindow):
    ...
    def _apply_state(self, state):
        ...
        log_steps = self._collect_ai_steps_from_logs(new_log_lines, state)
    ...
    def _collect_ai_steps_from_logs(self, new_lines, state):
        steps = []
        default_phase = self._normalize_phase_key(state.get("phase") if state else None)
        if state and state.get("active") == "model":
        ...
        for line in new_lines:
            ...
            if not self._ai_turn_active and default_phase is None:
                continue

            unit_id = self._unit_id_from_log(text)
            if unit_id is None:
                continue
            phase_for_unit = self._ai_current_phase or self._ai_phase_override or default_phase
            if not phase_for_unit:
                self.add_log_line(
                    "Ход ИИ: пропущен шаг — не определена фаза для строки "
                    f"с unit_id={unit_id}. Что делать дальше: проверьте лог фазы."
                )
                continue
            if not self._should_add_unit_step(text, phase_for_unit):
                self.add_log_line(
                    "Ход ИИ: пропущен шаг — строка не относится к фазе "
                    f"{phase_for_unit} для unit_id={unit_id}."
                )
                continue
            steps.append({"phase_key": phase_for_unit, "unit_id": unit_id, "raw": text})
        return steps
