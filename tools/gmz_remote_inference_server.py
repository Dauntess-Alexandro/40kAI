#!/usr/bin/env python
"""Standalone remote GMZ inference server (ZMQ ROUTER + GPU batch infer)."""
...
from core.models.gmz_inference_server import GMZInferenceServer  # noqa: E402
from core.models.gumbel_muzero_model import GumbelMuZeroNet  # noqa: E402
from core.models.gumbel_muzero_search import GumbelMuZeroSearchConfig  # noqa: E402
...
class GMZRemoteInferenceServer:
    def __init__(
        self,
        *,
        host: str,
        ...
        if weight_src:
            ...
            _log(f"[GMZ][REMOTE_IS] loaded init weights from {weight_src}", log_path)
        elif os.path.isfile(self.weights_path):
            payload = torch.load(self.weights_path, map_location="cpu", weights_only=False)
            sd = payload.get("state_dict") if isinstance(payload, dict) else payload
            net.load_state_dict(normalize_state_dict(sd), strict=False)
            _log(f"[GMZ][REMOTE_IS] loaded weights from {self.weights_path}", log_path)
        else:
        ...
        search_config = GumbelMuZeroSearchConfig(
            num_simulations=int(search_cfg.get("num_simulations", 32)),
            root_top_k=int(search_cfg.get("root_top_k", 8)),
            discount=float(search_cfg.get("discount", 0.997)),
            temperature=float(search_cfg.get("temperature", 0.15)),
            gumbel_scale=float(search_cfg.get("gumbel_scale", 1.0)),
            prior_weight=float(search_cfg.get("prior_weight", 0.25)),
            batch_recurrent=bool(int(search_cfg.get("batch_recurrent", 1))),
            tree_reuse=bool(int(search_cfg.get("tree_reuse", 1))),
        )
        ...
        self._engine = GMZInferenceServer(
            net=net,
        ...
        _log(f"[GMZ][REMOTE_IS] listening on {self.host}:{self.port} device={torch_device}", log_path)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="40kAI GMZ Remote Inference Server")
    ...
    parser.add_argument("--search-config", default=None, help="JSON with obs_dim, action_sizes, sims…")
    ...
    if log_path is None:
        ...
        log_path = logs_dir / f"gmz_remote_is_{stamp}.log"
