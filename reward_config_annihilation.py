"""Reward-профиль миссии Annihilation / Kill Points (самодостаточный)."""
...
# Annihilation: objective/VP-shaping нейтрализован (миссия считает Kill Points).
# Бонус за удержание objective.
VP_OBJECTIVE_HOLD_REWARD = 0.0
# Штраф за упущенное удержание/прогресс по objective.
VP_OBJECTIVE_HOLD_PENALTY = 0.0
# Бонус за приближение к objective.
VP_OBJECTIVE_PROXIMITY_REWARD = 0.0
# Нормировочный знаменатель для штрафа за пропущенный прогресс (делитель — НЕ зануляем):
...
TURN_LIMIT_VP_MARGIN_PENALTY_SCALE = 0.0
# Ограничение абсолютного VP margin для turn_limit shaping (clamp — НЕ зануляем).
TURN_LIMIT_VP_MARGIN_CLAMP = 6.0
# Длина серии удержания objective для streak-бонуса (threshold — НЕ зануляем).
VP_OBJECTIVE_STREAK_LEN = 1
# Дополнительный бонус за достижение серии удержания objective.
VP_OBJECTIVE_STREAK_BONUS = 0.0
# Масштаб награды за изменение перевеса OC на objectives (delta по шагу).
VP_OBJECTIVE_OC_MARGIN_SCALE = 0.0
...
# =============================
# Масштаб награды за нанесённый урон в стрельбе. v2 (kill-tuned): 0.6->0.45.
SHOOT_REWARD_DAMAGE_SCALE = 0.45
# Фиксированный бонус за убийство модели/цели стрельбой. v2 (kill-tuned): 0.4->0.9 (финиш = KP).
SHOOT_REWARD_KILL_BONUS = 0.9
# Штраф за избыточный урон (overkill). v2 (kill-tuned): 0.2->0.35 (потерянный второй килл).
SHOOT_REWARD_OVERKILL_PENALTY = 0.35
# Штраф за пропуск стрельбы, когда действие ожидалось. v2 (kill-tuned): 0.15->0.20 (анти-пассивность).
SHOOT_REWARD_SKIP_PENALTY = 0.20
# Доп. бонус за выбор цели с низким HP. v2 (kill-tuned): 0.05->0.20 (добивать раненых в KP).
SHOOT_REWARD_TARGET_LOW_HP = 0.20
...
# v2 (kill-tuned): 0.32->0.45 (свои потери = KP врагу + хуже destroyed_hp).
DAMAGE_TAKEN_SCALE = 0.45

# ==============================================
# Шэйпинг наград (objectives / utility-сигналы)
# ==============================================
# Annihilation: objective-utility shaping нейтрализован.
# Штраф за бездействие вне целей (idle out of objective).
IDLE_OUT_OF_OBJECTIVE_PENALTY = 0.0
# Масштаб пошагового бонуса за прогресс к objective.
OBJECTIVE_PROGRESS_STEP_SCALE = 0.0
# Верхняя граница бонуса за прогресс к objective за шаг.
OBJECTIVE_PROGRESS_STEP_CAP = 0.0
# Штраф за "мертвое окно": нет валидных целей стрельбы и нет contest objective.
NO_TARGET_NO_CONTEST_PENALTY = 0.0
# Рост штрафа за "мертвое окно" по раундам после старта скоринга (round_scale — НЕ зануляем).
NO_TARGET_NO_CONTEST_ROUND_SCALE = 0.03
# Верхняя граница мультипликатора раундового роста для "мертвого окна" (max_mult — НЕ зануляем).
NO_TARGET_NO_CONTEST_MAX_MULT = 2.0
# Бонус за убийство цели, находящейся на objective.
KILL_ON_OBJECTIVE_BONUS = 0.0
# Масштаб бонуса за нанесение урона цели на objective.
DAMAGE_ON_OBJECTIVE_SCALE = 0.0
...
# ==========================================
# Annihilation: mission-no-contest shaping нейтрализован (KP — источник истины).
# Доп. штраф, если после старта скоринга никто не contest'ит objectives.
MISSION_NO_CONTEST_PENALTY = 0.0
# С какого battle round включать этот штраф (start_round — НЕ зануляем).
MISSION_NO_CONTEST_START_ROUND = VP_START_SCORING_ROUND
# После этого раунда штраф мягко усиливается (late_round — НЕ зануляем).
MISSION_NO_CONTEST_LATE_ROUND = 10
# +15% к базовому штрафу после LATE_ROUND (late_mult — НЕ зануляем).
MISSION_NO_CONTEST_LATE_MULT = 1.25

# Anti-stall по VP diff: если серия шагов без изменения VP слишком длинная.
# Annihilation: VP-stall shaping нейтрализован (KP пересчётом, не VP-поток).
VP_STALL_STEPS_THRESHOLD = 5
VP_STALL_PENALTY = 0.0
...
# =========================
# Масштаб награды за нанесённый урон в ближнем бою. v2 (kill-tuned): 0.6->0.45.
MELEE_REWARD_DAMAGE_SCALE = 0.45
# Фиксированный бонус за убийство в ближнем бою. v2 (kill-tuned): 0.4->0.9 (финиш = KP).
MELEE_REWARD_KILL_BONUS = 0.9
# Масштаб штрафа за полученный урон в ближнем бою. v2 (kill-tuned): 0.5->0.6 (charge-trade осторожнее).
MELEE_REWARD_TAKEN_SCALE = 0.6
...
ENEMY_HEUR_OBJECTIVE_DIST_W = 0.0   # annihilation: оппонент не идёт к фантом-точке
ENEMY_HEUR_OBJECTIVE_CONTROL_W = 0.0   # annihilation: точку не контролируем (её нет в скоринге)
...
ENEMY_HEUR_OBJECTIVE_PRESSURE_W = 0.0   # annihilation: нет давления на точку
...
ENEMY_HEUR_OBJECTIVE_CONTROL_ENABLED = 0   # annihilation: слой контроля точки выключен
...
ENEMY_HEUR_SHOOT_OBJECTIVE_W = 0.0   # annihilation: выбор цели без бонуса «на точке»
ENEMY_HEUR_SHOOT_OVERKILL_W = 0.18
ENEMY_HEUR_CHARGE_MATCHUP_W = 0.40
ENEMY_HEUR_CHARGE_DISTANCE_W = 0.35
ENEMY_HEUR_CHARGE_OBJECTIVE_W = 0.0   # annihilation: чардж без бонуса «на точке»
