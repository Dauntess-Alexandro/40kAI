...
class RollLogger:
    ...
    *количество выстрелов* (если профиль "D6" и т.п.). Тогда порядок бросков смещается.
    ...
    def print_melee_report(self, weapon: dict, attacker_data: dict, defender_data: dict, dmg_list, effect=None):
        ...
        if sv is not None:
            inv_txt = f"{inv}+" if (inv is not None and int(inv) > 0) else "нет"
        ...
        if s is not None and t is not None and wound_rolls:
            ...
            wounds = sum(1 for r in wound_rolls if r != 1 and r >= wt)
    ...
    def enemyTurn(self, trunc=False):
        ...
        for i in range(len(self.enemy_health)):
            ...
            if self.enemyInAttack[i][0] == 0 and self.enemy_health[i] > 0:
                ...
                ## ===== Fight Phase (10e) =====
                self.resolve_fight_phase(active_side="enemy")
                ...
                if advanced:
                    ...
                    if len(chargeAble) > 0:
                        ...
                        if trunc is False:
                            wname = self.enemy_melee[i].get("Name", "Melee") if isinstance(self.enemy_melee[i], dict) else str(self.enemy_melee[i])
                            print(f"⚔️ Enemy Unit {enemyName} fights Model Unit {idOfM + 11} with {wname}: dmg {float(np.sum(dmg))} | HP {hp_before} -> {modHealth}")

                        if trunc is False and _logger is not None:
                            _logger.print_melee_report(
                                weapon=self.enemy_melee[i],
                        ...
                        while response is False:
                            ...
                            if is_num(attk) is True and int(attk) - 21 in charg:
                                ...
                                if distance(self.enemy_coords[i], self.unit_coords[j]) - sum(roll) <= 5:
                                    ...
                                    self.enemyCharged[i] = 1   # важно для "chargers first" в Fight Phase
