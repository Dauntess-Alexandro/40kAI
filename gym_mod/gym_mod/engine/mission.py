...
def _generate_only_war_terrain_features(b_len: int, b_hei: int, *, rng: random.Random) -> list[TerrainFeature]:
    ...
    # Фиксированная пара «бочек» у маркеров в центральной зоне (как во Viewer).
    # Ставим ДО случайной генерации, чтобы случайные пары не заняли эти клетки.
    fixed_left_col = int(max(1, min(b_hei - 2, 10)))
    fixed_right_col = int((b_hei - 1) - fixed_left_col)
    row_candidates = [
        int(max(1, min(b_len - 4, (b_len // 2) - 5))),
        int(max(1, min(b_len - 4, (b_len // 2) - 4))),
        int(max(1, min(b_len - 4, (b_len // 2) - 3))),
    ]
    for marker_row in row_candidates:
        marker_left = _make_barricade_cells(marker_row, fixed_left_col, "vertical")
        marker_right = _make_barricade_cells(marker_row, fixed_right_col, "vertical")
