...
def convertToDict(action, len_model: int | None = None):
    ...
    if len_model is None:
        # Формула после Task 4 (use_cp/cp_on убраны из контракта):
        # BASE = 2 (move, attack); per-unit = 3×n; strat = 2×len(STRATAGEM_PHASES)
        # total = 2 + 3×n + 2×strat → n = (total - 2 - strat_keys) // 3
        from core.engine.phases.stratagems import STRATAGEM_PHASES
        strat_keys = 2 * len(STRATAGEM_PHASES)
        len_model = max(0, (len(naction) - 2 - strat_keys) // 3)
