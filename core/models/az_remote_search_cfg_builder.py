"""AZ remote search_cfg: publish/ensure для remote IS (ПК1↔ПК2)."""
...
SEARCH_CFG_NAME = "az_remote_search_cfg.json"
WEIGHTS_NAME = "latest_az_tree_policy.pth"


def resolve_az_smb_paths(share_root: str):
    return resolve_smb_paths(share_root, search_cfg_name=SEARCH_CFG_NAME, weights_name=WEIGHTS_NAME)


def _load_hyperparams_az() -> dict[str, Any]:
...
def publish_az_remote_search_cfg_from_repo(
    *,
    ...
    if obs_dim <= 0 or not action_sizes:
        raise FileNotFoundError("AZ: obs_dim/action_sizes не определены из ростера/checkpoint.")

    az = _load_hyperparams_az()
...
def ensure_az_remote_search_cfg(share_root: str):
    return ensure_remote_search_cfg(
        share_root,
        search_cfg_name=SEARCH_CFG_NAME,
        publish_from_repo=lambda **kw: publish_az_remote_search_cfg_from_repo(
            sources=["ensure_az_remote_search_cfg:auto"],
            **kw,
        ),
