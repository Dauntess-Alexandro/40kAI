"""Centralized GPU inference server for Sampled MuZero (variant B)."""
...
class SMZInferenceServer:
    """Батчит запросы env workers, выполняет BatchedSampledMuZeroSearch на GPU."""
    ...
    def __init__(
        ...
        *,
        ...
        self._batched_search: BatchedSampledMuZeroSearch | None = None
        ...
        if compile_mode and device.type == "cuda" and hasattr(torch, "compile"):
            if not _triton_available():
                _append_log(
                    "[SMZ][INF_SERVER] torch.compile skipped: triton not installed "
                    "(normal on Windows; set SMZ_REMOTE_COMPILE=0 to hide this)"
                )
            else:
                try:
                    self.net = torch.compile(self.net, mode="reduce-overhead", fullgraph=False)
                    _append_log("[SMZ][INF_SERVER] torch.compile enabled (mode=reduce-overhead)")
                except Exception as exc:
                    _append_log(f"[SMZ][INF_SERVER] torch.compile skipped: {exc}")

        # Батч-поиск по средам: создаём ПОСЛЕ compile, чтобы получить скомпилированную сеть.
        self._batched_search = BatchedSampledMuZeroSearch(
                ...
                if os.path.isfile(self.sync_path):
                    ...
                    if mtime > last_mtime:
                        ...
                        if isinstance(sd, dict):
                            ...
                            _append_log(f"[SMZ][INF_SERVER] weight_updated version={new_ver}")
            except Exception as exc:
                _append_log(f"[SMZ][INF_SERVER] weight_poll_error: {exc}")
    ...
    def _log_batch_stats(self, batch_size: int, responses: list[dict[str, Any]]) -> None:
        ...
        if batch_size > 1 or elapsed_ms > 200.0:
            _append_log(
                f"[SMZ][INF_SERVER] batch={batch_size} inference_ms={elapsed_ms:.1f} "
                f"total_reqs={self._requests_total}"
            )
        if elapsed_ms > 500.0:
            _append_log(f"[SMZ][INF_SERVER] slow_batch ms={elapsed_ms:.1f} for n={batch_size}")
        if not responses:
            return


def smz_inference_server_entry(
    request_q: Any,
    reply_queues: list[Any],
    sync_path: str,
    init_weights: dict,
    ...
    *,
    ...
    server: SMZInferenceServer | None = None
    try:
        ...
        _append_log(
            f"[SMZ][INF_SERVER] started batch={inference_batch_size} "
            f"device={device.type} workers={len(reply_queues)}"
        )
        server.run()
    except Exception as exc:
        _append_log(f"[SMZ][INF_SERVER] fatal: {exc}")
        raise
    finally:
