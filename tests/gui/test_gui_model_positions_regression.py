...
class TestGuiModelPositionsRegression(unittest.TestCase):
    ...
    def test_units_layer_draws_icons_for_models_without_circles(self):
        source = Path("app/viewer/opengl_view.py").read_text(encoding="utf-8")
        self.assertIn("# Р‘РµР· РєСЂСѓРіРѕРІ: СЂРёСЃСѓРµРј С‚РѕР»СЊРєРѕ РёРєРѕРЅРєРё", source)
    ...
    def test_icon_scales_are_configurable_for_units_and_models(self):
        app_source = Path("app/viewer/app.py").read_text(encoding="utf-8")
        view_source = Path("app/viewer/opengl_view.py").read_text(encoding="utf-8")
