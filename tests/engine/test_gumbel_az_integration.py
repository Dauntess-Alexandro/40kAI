...
def test_gumbel_az_env_rollout_valid_targets_and_restore():
    len_model = 1
    n_obs = 16
    # Task 4: use_cp/cp_on убраны из base. Новый контракт для len_model=1:
    # [move, attack, move_num_0, shoot_num_0, charge_num_0] + strat_command(2,1)+movement(2,1)+shooting(3,1)+charge(2,1)+fight(4,1) = 15
    n_actions = [5, 2, 5, 3, 24, 2, 1, 2, 1, 3, 1, 2, 1, 4, 1]
...
def test_gumbel_az_prefers_winning_attack():
    ...
    len_model = 1
    n_obs = 12
    # Task 4: use_cp/cp_on убраны. [move, attack, move_num_0, shoot_num_0, charge_num_0] + strat heads.
    n_actions = [5, 2, 5, 2, 24, 2, 1, 2, 1, 3, 1, 2, 1, 4, 1]
    net = AlphaZeroPolicyValueNet(n_obs, n_actions)
    env = _FakeTreeEnv(n_obs=n_obs, n_actions=n_actions, len_model=len_model)
