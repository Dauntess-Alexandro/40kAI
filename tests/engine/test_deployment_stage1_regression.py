...
def _load_mission_module(module_name: str, io_instance=None):
    logging_utils = types.ModuleType("core.engine.logging_utils")
    ...
    sys.modules.setdefault("core.engine", types.ModuleType("core.engine"))
    sys.modules["core.engine.logging_utils"] = logging_utils
    sys.modules["core.engine.game_io"] = game_io

    mission_path = Path("core/engine/mission.py")
...
class TestDeploymentStage1Regression(unittest.TestCase):
    ...
    def test_rl_phase_applies_to_model_only_player_side_keeps_manual(self):
        ...
        self.assertEqual(0.0, float(stats.get("reward", 0.0)))
...
class TestDeploymentStage2ManualRegression(unittest.TestCase):
    ...
    def test_manual_player_deploy_retries_and_can_fallback_to_auto(self):
        ...
        self.assertTrue(any("СЂСѓС‡РЅРѕР№ РІРІРѕРґ РѕС‚РјРµРЅС‘РЅ" in line for line in logs))
