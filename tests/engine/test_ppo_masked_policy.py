...
def test_ppo_masked_action_with_value_ensemble():
    net = ActorCriticMultiHead(n_observations=6, n_actions=[3, 4], hidden_size=32, num_layers=1, n_value_ensemble=2)
...
def test_ppo_masked_action_respects_invalid_entries_core():
    net = ActorCriticMultiHead(n_observations=6, n_actions=[3, 4], hidden_size=32, num_layers=1)
    obs = torch.zeros((1, 6), dtype=torch.float32)
    masks = [
        torch.tensor([[False, True, False]], dtype=torch.bool),
        torch.tensor([[True, False, False, False]], dtype=torch.bool),
