...
class TestShootingOverlayTargetsVisualRegression(unittest.TestCase):
    def test_targets_overlay_draw_function_exists(self):
        source = Path("app/viewer/opengl_view.py").read_text(encoding="utf-8")
    ...
    def test_hover_marker_and_hover_hit_test_present(self):
        ...
        self.assertIn('painter.drawText(marker_bg, QtCore.Qt.AlignCenter, "рџЋЇ")', source)
    ...
    def test_range_overlay_not_shrunk_by_target_filter(self):
        ...
        self.assertIn("РЅРµ СЃСѓР¶Р°РµРј range РґРѕ РЅР°Р±РѕСЂР° С‚РµРєСѓС‰РёС… РІР°Р»РёРґРЅС‹С… С†РµР»РµР№", source)
    ...
    def test_shoot_range_debug_logs_present(self):
        ...
        self.assertIn("Р§С‚Рѕ СЃР»СѓС‡РёР»РѕСЃСЊ:", source)
        self.assertIn("Р§С‚Рѕ РґРµР»Р°С‚СЊ РґР°Р»СЊС€Рµ:", source)
