...
class TestGuiModelPositionsRegression(unittest.TestCase):
    ...
    def test_units_layer_draws_icons_for_models_without_circles(self):
        ...
        self.assertIn("# Мульти-аура: мягкие точки под каждой моделью", source)
    ...
    def test_selection_and_active_fx_use_formation_not_center_only(self):
        source = Path("viewer/opengl_view.py").read_text(encoding="utf-8")
        self.assertIn("# Внешний контур формации вместо центрального круга.", source)
        self.assertIn("# Внутренний пульс по нескольким ключевым моделям", source)
        self.assertIn("pulse_indices = sorted({0, len(model_centers) // 2, len(model_centers) - 1})", source)
