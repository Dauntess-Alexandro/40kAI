...
## 2.2. A — Lean info для train-mode ✅ СДЕЛАНО
**Что делаем**
...
**Статус в репо**: реализовано в subprocess worker (`TRAIN_LEAN_INFO=1` по умолчанию, возврат облегчённого `info` для `step`).

---

## 2.3. C — упрощение reward-heavy части step ✅ СДЕЛАНО
**Что делаем**
...
**Статус в репо**: реализовано через `REWARD_SECONDARY_INTERVAL` (по умолчанию `1`, без изменения текущего поведения); тяжёлые secondary-проверки в reward можно выполнять реже.

---

## 2.4. J — реальный async replay prefetch (поток) ✅ СДЕЛАНО
**Что делаем**
...
**Статус в репо**: prefetch вынесен в фоновые потоки в replay memory с блокировками для thread-safe `push/sample/update_priorities`.
