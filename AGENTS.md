...
- **Конфиг:** env `OPPONENT_POOL_*` → секция `opponent_pool` в `hyperparams.json` → default. Поля секции: `enabled`, `p_heuristic`, `pool_size`, `strategy` (`pfsp`|`uniform`), `pfsp_power`, `uniform_floor`, `novelty_bonus`, `min_games_for_pfsp`, `ema_alpha`, `seed`, `league_snapshot_every` (дефолт 100).
- **Env-флаги:** `OPPONENT_POOL_ENABLED`, `OPPONENT_POOL_P_HEURISTIC`, `OPPONENT_POOL_SIZE`, `OPPONENT_POOL_STRATEGY`, `OPPONENT_POOL_PFSP_POWER`, `OPPONENT_POOL_UNIFORM_FLOOR`, `OPPONENT_POOL_NOVELTY_BONUS`, `OPPONENT_POOL_MIN_GAMES`, `OPPONENT_POOL_EMA_ALPHA`, `OPPONENT_POOL_SEED`, `OPPONENT_POOL_LEAGUE_SNAPSHOT_EVERY`.
- **Модули:** общий torch-free слой — `core/engine/opponent_pool.py` (`PoolConfig`, `resolve_pool_config`, `OpponentStatsStore`, `OpponentPool`, `OpponentChoice`); тонкое per-algo потребление — `core/models/opponent_pool_runtime.py` (`OpponentRuntimeCache`, `OpponentPoolStatsWriter`, `LeagueLearnerSnapshotWriter`, `build_pool_for_actor`, `default_candidate_provider`). Врезка в `train.py` (PPO/DQN/AZ/GAZ/GMZ/SMZ): при `SELF_PLAY_ENABLED=1` и `POOL_CONFIG.enabled` — сэмпл на эпизод; акторы шлют `pool_result`, а persistent stats обновляет только единый writer learner-процесса (включая distributed transport). При включённом пуле learner дополнительно пишет снапшоты в реестр каждые `league_snapshot_every` кумулятивных эпизодов + финальный (`final_epN`), отдельно от mirror-sync.
...
## Правило коммитов после внедрения
- После завершения внедрения фичи или фикса — сразу делать коммит проверенных изменений (не ждать «Все ок»).
- Один логический блок работы — один коммит; так проще отслеживать историю.
- Если пользователь в том же сообщении (или следующим сообщением) даёт новую задачу («теперь делаем ...»), агент должен:
  1) сначала закоммитить уже завершённые изменения,
