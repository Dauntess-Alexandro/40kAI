...
## Логи для отладки (logs/LOGS_FOR_AGENTS_TRAIN.md / logs/LOGS_FOR_AGENTS_PLAY.md)
- В папке `logs/` есть два файла логов: **LOGS_FOR_AGENTS_TRAIN.md** (train/eval) и **LOGS_FOR_AGENTS_PLAY.md** (игра/Viewer) — это “источник правды” по тому, что реально происходило.
- При любом баге/неожиданном поведении:
  1) **сначала прочитай свежие строки** из `logs/LOGS_FOR_AGENTS_TRAIN.md` и `logs/LOGS_FOR_AGENTS_PLAY.md`,
...
- **Можно смотреть `results.txt`** для быстрой сверки итогов train/eval.
