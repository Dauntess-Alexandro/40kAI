...
func _submit_command(text: String) -> void:
    ...
    if trimmed == "":
        ...
        return
    command_prompt.text = "Команда: %s" % trimmed
    command_input.clear()
...
func _apply_units_table(units_value) -> void:
    ...
    for unit in units:
        ...
        _add_units_cell(_to_text(unit_data.get("id", "—")))
        _add_units_cell(_format_side(unit_data.get("side")))
        _add_units_cell(_to_text(unit_data.get("hp", "—")))
        _add_units_cell(_to_text(unit_data.get("models", "—")))
...
func _add_units_cell(text: String) -> void:
    var label := Label.new()
    label.text = text
