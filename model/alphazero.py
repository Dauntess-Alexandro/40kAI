...
def canonical_action_keys(action_space) -> list[str]:
    base = ["move", "attack", "shoot", "charge", "use_cp", "cp_on"]
    keys = [k for k in base if k in action_space.spaces]
    move_num = [k for k in action_space.spaces.keys() if k.startswith("move_num_")]
    move_num.sort(key=lambda name: int(name.split("_")[-1]) if name.split("_")[-1].isdigit() else 0)
    return keys + move_num


def action_space_layout(action_space) -> tuple[list[str], list[int]]:
    keys = canonical_action_keys(action_space)
...
def to_action_dict(action_keys: list[str], action_tuple: tuple[int, ...]) -> dict[str, int]:
    if len(action_tuple) < len(action_keys):
        padded = list(action_tuple) + [0] * (len(action_keys) - len(action_tuple))
        action_tuple = tuple(padded)
